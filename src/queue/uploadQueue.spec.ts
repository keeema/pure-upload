describe("uploadQueue", () => {
  ...
  describe("basic logic", () => {
    ...
    it("sets the 'queued' state for newly added files", () => {
      ...
      expect(uploadQueue.queuedFiles[0].uploadStatus).toEqual(
        UploadStatus.queued
      );
      expect(uploadQueue.queuedFiles[1].uploadStatus).toEqual(
        UploadStatus.queued
      );
    });
  ...
  describe("callbacks", () => {
    ...
    it("triggers onFileAddedCallback and queueChangedCallback on add", () => {
      ...
      expect(queueChangedCallbackSpy).toHaveBeenCalledWith(
        uploadQueue.queuedFiles
      );
  ...
  describe("filesChanged", () => {
    describe("autoRemove", () => {
      ...
      it("removes finished files when autoRemove is turned on", () => {
        ...
        expect(uploadQueue.queuedFiles[2].uploadStatus).toEqual(
          UploadStatus.failed
        );
      });
    });
    ...
    describe("autoStart", () => {
      ...
      it("starts all queued files when there is no limit and autoStart is turned on", () => {
        ...
        expect(uploadQueue.queuedFiles[3].uploadStatus).toEqual(
          UploadStatus.uploading
        );
        expect(uploadQueue.queuedFiles[4].uploadStatus).toEqual(
          UploadStatus.uploading
        );
        expect(uploadQueue.queuedFiles[5].uploadStatus).toEqual(
          UploadStatus.uploaded
        );
        expect(uploadQueue.queuedFiles[6].uploadStatus).toEqual(
          UploadStatus.failed
        );
        expect(uploadQueue.queuedFiles[7].uploadStatus).toEqual(
      ...
      it("starts only limited count of files when set limit and autoStart is turned on", () => {
        uploadQueue = new UploadQueue(
          { autoStart: true, maxParallelUploads: 2 },
          {}
        );
        files.forEach(file => uploadQueue.queuedFiles.push(file));
        uploadQueue["filesChanged"]();
      ...
      it("starts limited count of files with offset when set limit and parallelBatchOffset and autoStart is turned on", (done: Function) => {
        uploadQueue = new UploadQueue(
          { autoStart: true, maxParallelUploads: 2, parallelBatchOffset: 1000 },
          {}
        );
        files.forEach(file => uploadQueue.queuedFiles.push(file));
