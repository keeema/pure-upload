...
class QueueRenderer {
  ...
  createQueueRow(
    file: pu.IUploadFile,
    queueSettings: pu.IUploadQueueOptions
  ): HTMLElement {
  ...
  renderQueue(
    ...
    files.forEach((file: pu.IUploadFile) => {
      if (queue) queue.appendChild(this.createQueueRow(file, queueSettings));
    });
  }
}
